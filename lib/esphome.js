...
class EspHomeClient {
  constructor ({ host, username, password, timeoutMs = DEFAULT_TIMEOUT_MS }) {
    ...
    this.headers = {}
    // A password without a username is for the prebuilt firmware's "admin" user.
    if (username || password) {
      const token = Buffer.from(`${username || 'admin'}:${password || ''}`).toString('base64')
      this.headers.Authorization = `Basic ${token}`
    }
  }

  // Sends a request with the given headers (default: the configured
  // credentials) and returns the response whatever its status.
  send (method, path, headers = this.headers) {
  ...
  async request (method, path) {
    ...
    if (!res.ok) {
      ...
      const err = new Error(`${method} ${path} returned HTTP ${res.status}`)
    ...
    const run = async () => {
      ...
      while (!stopper.signal.aborted) {
        ...
        try {
          ...
          if (!res.ok) {
            const err = new Error(`GET /events returned HTTP ${res.status}`)
            err.status = res.status
            throw err
          }
          onStatus(true)
